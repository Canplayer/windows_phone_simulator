...
class StartMenu extends StatefulWidget {
  ...
  final int crossAxisCount; // 网格列数，默认 4
  ...
  @override
  StartMenuState createState() => StartMenuState();
}

class StartMenuState extends State<StartMenu> with TickerProviderStateMixin {
  //final int widget.crossAxisCount = 4;
  ...
  Future<void> startPushNextAnimations(String tapInstanceId) async {
    ...
    double cellWidth = renderBox.size.width / widget.crossAxisCount;
    ...
    const int pushTime = 350;
    const int singleTileTime = 150;
      ...
      Future.delayed(Duration(milliseconds: delayMs), () {
        if (mounted) {
          _flipControllers[tile.instanceId]?.duration =
              const Duration(milliseconds: singleTileTime);
          _flipAnimations[tile.instanceId] =
              Tween<double>(begin: 0.0, end: math.pi / 2)
  ...
  Future<void> startPopNextAnimations() async {
    ...
    const int pushTime = 350;
    const int singleTileTime = 150;
    const int popDuration = singleTileTime * 3;
    final int groupDelay = maxDistance == 0
        ? 0
        : ((pushTime - singleTileTime) / maxDistance).round();
    ...
    for (var tile in visibleTiles) {
      ...
      Future.delayed(Duration(milliseconds: delayMs), () {
        if (mounted) {
          _flipControllers[tile.instanceId]?.duration =
              const Duration(milliseconds: popDuration);
          _flipAnimations[tile.instanceId] =
  ...
  Offset _findEmptySpot(List<TileModel> currentTiles, int width, int height) {
    int cols = widget.crossAxisCount; // 桌面总列数
    int maxGridY = 0;
  ...
  void pinApp(App app) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final RenderBox? box =
          _stackKey.currentContext?.findRenderObject() as RenderBox?;
      if (box != null) {
        double cellSize = box.size.width / widget.crossAxisCount;
    ...
    if (directCollisions.isNotEmpty) {
      ...
      for (var dir in directions) {
        ...
        for (var item in directCollisions) {
          ...
          if (nx < 0 || nx + item.widthCells > widget.crossAxisCount) {
            isOutOfBoundsX = true;
            break;
          }
    ...
    if (draggingTileId == tile.instanceId &&
      ...
      setState(() {
        ...
        if (hasMetDragThreshold) {
          ...
          newGridX = newGridX.clamp(0, widget.crossAxisCount - tile.widthCells);
  ...
  void _onDragEnd(TileModel tile, double cellSize) {
    setState(() {
      ...
      if (tile.dragPixelOffset != null && hasMetDragThreshold) {
        ...
        finalX = finalX.clamp(0, widget.crossAxisCount - tile.widthCells);
      ...
      child: AnimatedScale(
        ...
        child: LayoutBuilder(
          builder: (context, constraints) {
            final double cellSize = constraints.maxWidth / widget.crossAxisCount;
                ...
                onPressed: () {
                  setState(() {
                    ...
                    activeTile.gridX = activeTile.gridX
                        .clamp(0, widget.crossAxisCount - activeTile.widthCells);
...
class FloatingWrapper extends StatefulWidget {
  ...
  @override
  FloatingWrapperState createState() => FloatingWrapperState();
}

class FloatingWrapperState extends State<FloatingWrapper> {
  double _dx = 0;
  double _dy = 0;
  int _durationMs = 300;
  Timer? _timer;
